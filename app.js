...
/* === 経路図（data/ にアップされた k*.geojson を使用）=== */
/*   ※ ファイル名にスペースがあるため、fetch時は encodeURI でURL化します */
const ROUTES = [
  { id:"r831", label:"8/31", slots:[
    { shortLabel:"18:00-", src:"data/k831 1800-1930.geojson" },
    { shortLabel:"19:30-", src:"data/k831 1930-2200.geojson" },
  ]},
  { id:"r91", label:"9/1", slots:[
    { shortLabel:"10:00-", src:"data/k91 1000-1200.geojson" },
    { shortLabel:"13:00-", src:"data/k91 1300-1700.geojson" },
    { shortLabel:"18:00-", src:"data/k91 1800-2200.geojson" },
  ]},
  { id:"r92", label:"9/2", slots:[
    { shortLabel:"11:30-", src:"data/k92 1130-1300.geojson" },
    { shortLabel:"13:30-", src:"data/k92 1330-1500.geojson" },
    { shortLabel:"15:30-", src:"data/k92 1530-1700.geojson" },
    { shortLabel:"18:40-", src:"data/k92 1840-2200.geojson" },
  ]},
];
...
async function showRouteBySrc(src){
  ...
  if(!src) return;
  const url = encodeURI(src); // スペース入りファイル名対応
  routeOverlays = await addGeoJsonAsOverlays(url, ROUTE_STYLE);
}
...
async function initMap(){
  try{
    ...
    // ===== 経路図UI =====
    const rPill=$("routePill"), rDrawer=$("routeDrawer"), rClose=$("routeClose");
    const r0831=$("rTab0831"), r0901=$("rTab0901"), r0902=$("rTab0902");
    const rList=$("routeSlotList");
    const openR=()=>rDrawer.style.display="block";
    const closeR=()=>rDrawer.style.display="none";
    rPill.addEventListener("click", openR); rClose.addEventListener("click", closeR);
    document.addEventListener("click", (e)=>{ if(!rDrawer.contains(e.target) && e.target!==rPill && !rPill.contains(e.target)) closeR(); });

    function buildRouteButtons(day){
      rList.innerHTML="";
      day.slots.forEach(slot=>{
        const btn=document.createElement("button");
        btn.className="slotbtn"; btn.textContent=slot.shortLabel;
        btn.addEventListener("click", async ()=>{
          await showRouteBySrc(slot.src); // 青線
          [...rList.children].forEach(c=>c.classList.remove("active")); btn.classList.add("active");
        });
        rList.appendChild(btn);
      });
    }
    r0831.addEventListener("click", ()=>{ r0831.classList.add("active"); r0901.classList.remove("active"); r0902.classList.remove("active"); buildRouteButtons(ROUTES[0]); });
    r0901.addEventListener("click", ()=>{ r0901.classList.add("active"); r0831.classList.remove("active"); r0902.classList.remove("active"); buildRouteButtons(ROUTES[1]); });
    r0902.addEventListener("click", ()=>{ r0902.classList.add("active"); r0831.classList.remove("active"); r0901.classList.remove("active"); buildRouteButtons(ROUTES[2]); });
